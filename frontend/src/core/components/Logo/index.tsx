interface LogoProps {
  size?: number
  /** show the rounded gradient tile behind the mascot */
  tile?: boolean
}

/**
 * VyLing mascot — a friendly chat-bubble character with a little sprout,
 * representing language + growth. Replaces the old frog logo.
 */
export default function Logo({ size = 40, tile = true }: LogoProps) {
  const id = 'vy'
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" aria-label="VyLing">
      <defs>
