...
const KO_LINES = koLines as Record<string, EnLines>

function withLines(list: Scenario[], lines: Record<string, EnLines>): Scenario[] {
  return list.map((s) => {
    const l = lines[s.id]
    if (!l) return s
    return {
...
const BY_LANG: Record<string, Scenario[]> = {
  ko: withLines([...KO_SCENARIOS, ...extraScenarios((id) => KO_EXTRA_NAMES[id] || '지우')], KO_LINES),
  en: withLines([...EN_SCENARIOS, ...extraScenarios((id) => EN_EXTRA_NAMES[id] || 'Alex')], EN_LINES),
}
