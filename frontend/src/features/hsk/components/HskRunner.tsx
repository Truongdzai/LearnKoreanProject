...
import { speakZH, stopSpeak } from '@/core/tts'
import { track } from '@/core/monitor'
import { HSK_SKILLS, estimateHsk, type HskListening, type HskReading } from '@/data/hskCore'
import PinyinText from './PinyinText'

export type RunItem =
  | { kind: 'read'; item: HskReading }
  | { kind: 'listen'; item: HskListening }
...
function playScript(lines: HskListening['script']) {
  ...
  const speakAt = (k: number) => {
    if (token !== scriptToken || k >= lines.length) return
    speakZH(lines[k].text)
    window.setTimeout(() => speakAt(k + 1), Math.max(1800, lines[k].text.length * 400))
  }
  speakAt(0)
}

export default function HskRunner({ items, exam = false, seconds, title, onFinish, onQuit }: Props) {
  ...
  useEffect(() => () => { scriptToken += 1; stopSpeak() }, [])

  useEffect(() => {
    setShowScript(false)
    if (cur?.kind === 'listen') {
      const t = window.setTimeout(() => playScript(cur.item.script), 300)
  ...
  const submit = () => {
    ...
    scriptToken += 1
    stopSpeak()
    track('exam_finish', { exam: 'hsk', total: result.total, correct: result.correct })
    onFinish(result)
  }
  ...
  return (
    <div className="topik-run">
      <div className="topik-run-head">
        <button className="btn-ghost sm" onClick={() => { scriptToken += 1; stopSpeak(); onQuit() }}>
      ...
      <div className="topik-card">
        <div className="topik-kind">
          {cur.kind === 'listen' ? <><Icon name="headphones" size={14} /> 听力 · Nghe</> : <><Icon name="book" size={14} /> 阅读 · Đọc</>}
          <span className="topik-skill">{HSK_SKILLS[qSkill(cur)] ?? qSkill(cur)}</span>
        ...
        {cur.kind === 'listen' ? (
          <div className="topik-listen">
            <button className="btn-primary sm" onClick={() => playScript(cur.item.script)}>
            ...
            {showScript && (
              <div className="topik-script">
                {cur.item.script.map((l, k) => (
                  <div key={k}><b>{l.s === 'W' ? '女' : '男'}:</b> <PinyinText text={l.text} show={pinyin} /></div>
                ))}
              </div>
            )}
          </div>
        ) : (
          <div className="topik-text"><PinyinText text={cur.item.text} show={pinyin} /></div>
        )}
    ...
    <div className="quiz-done topik-done">
      ...
      {exam && (
        <div className="topik-est">
          <div><b>{est.listening}</b><span>听力 (ước tính /100)</span></div>
          <div><b>{est.reading}</b><span>阅读 (ước tính /100)</span></div>
      ...
      {!!weak.length && (
        <div className="topik-weak">
          ...
          <div className="topik-weak-list">
            {weak.map(([sk, v]) => (
              <span key={sk} className="topik-weak-item">{HSK_SKILLS[sk] ?? sk} <i>{v.ok}/{v.n}</i></span>
            ))}
          </div>
        </div>
      )}
